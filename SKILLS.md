...
5. **Fetches global market indicators** — CBOE VIX, Taiwan Buffett Indicator, and **Taiwan market regime** (TWII vs MA200)
6. **Applies market regime adjustment** — adds a score bonus/penalty based on where TWII stands relative to its MA200 so the strategy doesn't fight the tape
7. **Scores and recommends** a position-sizing action for the coming week
...
| 總體市場指標 | VIX fear index + Taiwan Buffett Indicator + **TWII vs MA200 regime** |
...
| 建議 | Position-sizing recommendation from 強力減碼 ⚠ to 強力加碼 🚀 (includes regime adjustment) |
...
- **Market regime bonus**: TWII position vs MA200 adds ±0.25/0.15/0/-0.15 to the combined score.
- **Combined**: `tech_score + inst_score × 0.12 + regime_bonus`

**In 強多頭 regime** (TWII ≥ MA200 × 1.05):
- Sell threshold raised from -0.25 → -0.40 (needs stronger signal to reduce)
- Strong-sell threshold raised from -0.60 → -0.80
- Price-move threshold 1.5% → 2% (filters noise in trending markets)
